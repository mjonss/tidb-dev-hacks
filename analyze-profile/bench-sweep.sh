...
playground_start() {
  ...
  render_memory_configs
  # TiFlash and the monitor stack (prometheus + grafana) are on by default in
  # tiup playground, but nothing here uses them: the benchmarked table has no
  # TiFlash replica, and analyze-profile scrapes TiDB's and TiKV's own /metrics
  # status ports directly (collectors.go) rather than querying prometheus. They
  # only compete for RAM and CPU with the process under measurement, which
  # matters on the async=OFF cells that peak near 30 GB. Default both off;
  # override with TIFLASH_NUM=1 / PLAYGROUND_MONITOR=1 if ever needed.
  local -a extra_args=(--tiflash "${TIFLASH_NUM:-0}")
  [[ "${PLAYGROUND_MONITOR:-0}" == "1" ]] || extra_args+=(--without-monitor)
  log "Starting fresh playground (tidb=${bin}, tiflash=${TIFLASH_NUM:-0}, monitor=${PLAYGROUND_MONITOR:-0})"
...
# the benchmarked run and pollute timings or stats. Also raise the global
# lock-wait-timeout from its 1s default so internal preflight flushes
# (e.g. ANALYZE TABLE's own FLUSH STATS_DELTA) have headroom when the
# background stats updater is still draining a large delta — without this
# a fresh seed-analyze on a 30M-row bulk-INSERT lock-times-out at <50s.
