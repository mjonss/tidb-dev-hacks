#!/usr/bin/env bash
# Run the full v5 matrix. Idempotent at (branch, scenario, async) group
...
# and this script doubles as its own resume. bench.sh's `run` truncates
# manifest.tsv, so we rebuild it from disk at the end before comparing.
#
# Launch detached (nohup+disown does NOT survive Claude session teardown — the
# harness SIGTERMs the child process group; and --user units need linger or the
# user manager stops them on a login-session change):
#   loginctl enable-linger "$USER"
#   systemd-run --user --unit=matrix-v5 --collect \
#     --working-directory=$PWD bash run-v5.sh
#
# Watch:
#   journalctl --user -u matrix-v5 -f --no-pager

set -uo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "${SCRIPT_DIR}"
OUT="${SCRIPT_DIR}/output-bench-v5"
...
echo "=== V5 MATRIX start $(date) ==="
echo "BASE = f994e8da27 (PR's true merge-base, resolved against pingcap/master)"
echo "PR   = 8cf6bd31b5"
./tidb-server.master -V | head -3
./tidb-server.pr -V | head -3
...
run_group() {
  ...
  echo "=== RUN ${label} ${scn} async=${asy} (2 iters) $(date) ==="
  SCENARIOS_FILTER="${scn}" STATES_FILTER="existing" ASYNC_MERGE="${asy}" \
    ./bench-config-v5.sh run "${scope}"
  echo "${label} ${scn} async=${asy} exit=$? $(date)"
}

# Cheap groups first, so a failure surfaces in minutes rather than hours.
run_group BASE part-single ON
run_group PR   part-single ON
run_group BASE part-single OFF
...
echo "=== Compare ==="
./bench-config-v5.sh compare > "${OUT}/report.txt" 2>&1
echo "compare exit=$? — report at ${OUT}/report.txt"
echo "=== V5 MATRIX done $(date) ==="
